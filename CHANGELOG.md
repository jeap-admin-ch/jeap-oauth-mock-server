...
### Removed
- Removed the explicit versions on `io.rest-assured:json-path` and `io.rest-assured:xml-path`: both used the parent's own `${rest-assured.version}` for artifacts `jeap-internal-spring-boot-parent` already manages.
...
- Add configurable OpenID Connect post-logout redirect URIs
...
- Use a two-letter ISO 3166-1 country code in the JWK signing certificate's subject/issuer DN, as enforced by Bouncycastle 1.85 (JEAP-7270)
...
- Disable inline event handlers on the login/logout forms to comply with the CSP from `jeap-spring-boot-web-config-starter`
- Disable browser caching of the login/logout forms by overriding the starter's `Cache-Control` headers
...
- Added requested scopes from the context to the ID token, enabling filtering of BPRoles when a BP-scoped profile is configured for the client.
